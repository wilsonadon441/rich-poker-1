"""Train the rich-poker-1 "boost" candidate model.

Gradient-boosting-heavy stack (LightGBM + XGBoost + ExtraTrees) with a
logistic meta-learner and hand-ngram side features. Holdout and recency
windows are derived dynamically from the freshest benchmark source dates so
the daily auto-retrain never goes stale. The candidate artifact is written to
artifacts/candidate.joblib; training/promote_if_better.py decides deployment.
"""
...
LGB_PARAMS = {
    "objective": "binary", "n_estimators": 900, "num_leaves": 47,
    "learning_rate": 0.03, "min_child_samples": 4, "subsample": 0.9,
    "colsample_bytree": 0.8, "reg_alpha": 0.2, "reg_lambda": 2.0,
    "random_state": SEED, "n_jobs": N_JOBS, "verbosity": -1,
}
XGB_PARAMS = {
    "n_estimators": 800, "max_depth": 6, "learning_rate": 0.035,
    "subsample": 0.9, "colsample_bytree": 0.8, "reg_alpha": 0.2,
    "reg_lambda": 2.0, "random_state": SEED, "n_jobs": N_JOBS,
    "objective": "binary:logistic", "eval_metric": "logloss",
    "verbosity": 0, "tree_method": "hist",
}
ET_PARAMS = {
    "n_estimators": 600, "max_depth": 14, "min_samples_leaf": 3,
    "max_features": 0.5, "random_state": SEED, "n_jobs": N_JOBS,
}
...
def main() -> int:
    ...
    for fold, (tr_idx, va_idx) in enumerate(kf.split(x_train, y_train)):
        ...
        for column, model in enumerate(fold_models):
            model.fit(x_train[tr_idx], y_train[tr_idx], sample_weight=sw_train[tr_idx])
    ...
    for model in prod_models:
        model.fit(x_all, y_all, sample_weight=sw_all)
